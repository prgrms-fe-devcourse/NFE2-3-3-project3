<template>
  <header class="bg-gray-100 shadow-md w-full">
    <div class="container mx-auto flex items-center pr-6 m-2 flex-wrap">
      <nav
        class="flex items-center space-x-3 text-gray-600 mt-2 lg:mt-0 lg:ml-auto -ml-2"
      >
        <a href="#" class="hover:text-gray-800 no-underline">다크 모드</a>
        <a href="#" class="hover:text-gray-800 no-underline">알림</a>
        <a href="#" class="hover:text-gray-800 no-underline">포인트</a>
        <a href="#" class="hover:text-gray-800 no-underline">로그아웃</a>
      </nav>
    </div>
  </header>
</template>
