...
const fetchQuestions = async () => {
  try {
    const response = await fetch(
      "https://dugout-programmers.github.io/Dugout/api/games/playerQuiz.json"
    ); // JSON 파일 경로
    if (!response.ok)
...
const submitAnswer = () => {
  ...
  try {
    isCorrect.value =
      currentQuestion.value.answer.toLowerCase().replace(/\s+/g, "") ===
    ...
    <div
      ...
      <div v-else>
        ...
        <div
          ...
          class="text-white01 w-[80%] flex flex-col gap-[20px] items-center absolute left-1/2 -translate-x-1/2 top-0">
          <div class="flex flex-col items-center gap-[10px]">
            <p class="font-Galmuri11">
              {{ currentIndex + 1 }}번째 문제! 선수 이름을 맞춰보세요
            </p>
            <p class="font-Galmuri11 text-[24px] break-keep text-center">
              <img :src="currentQuestion.question" class="w-[100px]" />
            </p>
          </div>
          <!-- 답안 입력 -->
