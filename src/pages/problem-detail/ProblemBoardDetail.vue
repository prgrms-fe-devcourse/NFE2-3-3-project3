...
const handleToggleLike = async () => {
  ...
  try {
    await problemLikeAPI.add(authStore.user.id, parseInt(route.params.problemId))
...
<template>
  <div class="max-w-4xl mx-auto p-6">
    ...
    <!-- 삭제 확인 다이얼로그 -->
    <Dialog v-model:visible="showConfirmDialog" header="삭제 확인" :modal="true">
      <p>정말로 이 게시물을 삭제하시겠습니까?</p>
      <template #footer>
        <Button label="취소" severity="secondary" @click="showConfirmDialog = false" />
        <Button label="삭제" severity="danger" @click="showConfirmDialog = false" />
      </template>
    </Dialog>
  </div>
</template>