...
const addParticipantByEmail = () => {
 ...
 const session = JSON.parse(localStorage.getItem('sb-' + import.meta.env.VITE_SUPABASE_PROJECT_REF + '-auth-token'));
...
watchEffect(() => {
 try {
   ...
   const supabaseAuthKey = storageKeys.find(key => key.startsWith('sb-') && key.endsWith('-auth-token'));
   ...
   if (supabaseAuthKey) {
     ...
     if (session?.user) {
       ...
       if (!hasOwner) {
         selectedUsers.value = [{
           email: user.email,
           nickname: user.user_metadata?.name || user.email?.split('@')[0] || "시험 주최자",
     ...
     <div class="rounded-xl bg-beige-1 w-full px-5 py-3 h-full flex-1">
       ...
       <ul class="overflow-auto divide-y divide-gray-100">
         <UserListItem
           ...
           :key="user.email"
           :user="user"
           :show-remove-button="selectedUsers.some((selected) => selected.email === user.email)"
           @add="handleUserAdd"
